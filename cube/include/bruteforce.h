...
public:
    BruteforceSolver(
        ...
        size_t ttip,
        int chsz,
        const std::string& reorder_strat = "sum_distance"
    ) : results(res), que(q), filters(filts), command(cmd), filename(fn), n(nn), k(kk),
        start_len(ttip), chunk_size(chsz), coloringsb(0), reorder_strategy(reorder_strat) {
...
private:
    // Метод Z: хэширует отсортированные add_verts, если size <= 128/n
    std::optional<__int128> Z(const std::vector<int>& add_verts) const {
        ...
        for (size_t i = 0; i < num; ++i) {
            __int128 vert_bits = static_cast<__int128>(sorted_add[i]) & ((static_cast<__int128>(1) << n) - 1);
            hash |= (vert_bits << (i * n));
        }
        return hash;
    }
    ...
    void bruteforce_nolimits(
        ...
        if (base_v.size() > static_cast<size_t>(start_len)) {
            ...
            if (maybe_hash) {
                std::pair<int, __int128> key{static_cast<int>(add_verts.size()), *maybe_hash};
                ...
                for (const auto& op : ops) {
                    ...
                    if (new_hash) {
                        std::pair<int, __int128> new_key{static_cast<int>(add_verts.size()), *new_hash};
    ...
    std::set<std::pair<int, __int128>> seen_configs;  // для дедупа
