...
int main(int argc, char** argv) {
    // Пути можно задать аргументами:  v2_n11k6_4_1 <путь к kissat> <рабочий каталог>
    ...
    const std::string FILENAME  = workdir + "/n11k6_4_1.txt";
    ...
    std::vector<int> starting_vertexes = {0, 0b11111100000, 0b11100011100, 0b00011111100 };

    std::vector<FuncPtr> filters;
    filters.push_back(is_inside);
    filters.push_back(is_not_created_k5);
    filters.push_back(is_not_created_k4_4_or_3_or_2);
    ...
    BruteforceSolver solver(results, que, filters, command_1 + FILENAME + command_2, FILENAME, N, K, starting_vertexes.size(), 201);
    solver.run();
