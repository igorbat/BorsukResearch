...
std::string FILENAME = "/root/science/bruteforce_via_kissat/Borsuk_plus_kissat/build/n12k8_2.txt";
std::string command_2 = " -q --time=600";
...
int main() {
    ...
    std::vector<int> starting_vertexes = {0, 0b111111110000 };

    std::vector<FuncPtr> filters;
    filters.push_back(is_inside);
    filters.push_back(is_not_created_k3);
    ...
    BruteforceSolver solver(results, que, filters, command_1 + FILENAME + command_2, FILENAME, N, K, starting_vertexes.size(), 150);
    solver.run();
